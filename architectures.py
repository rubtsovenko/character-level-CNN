...
def net_1(x, is_train):
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with slim.arg_scope([slim.conv2d], stride=1, padding='VALID'):
            ...
            net = slim.fully_connected(net, 12, activation_fn=None, scope='fc7')
    return net


def net_2(x, is_train):
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        activation_fn=tf.nn.relu,
                        weights_initializer=trunc_normal,
                        biases_initializer=const_init):
        with slim.arg_scope([slim.conv2d], stride=1, padding='VALID'):
            net = slim.conv2d(x, 256, [26,7], scope='conv1')
            net = slim.max_pool2d(net, [1,2], stride=2, scope='pool1')
            net = slim.conv2d(net, 256, [1, 6], scope='conv2')
            net = slim.max_pool2d(net, [1, 2], stride=2, scope='pool2')
            net = slim.conv2d(net, 256, [1, 4], scope='conv3')
            net = slim.conv2d(net, 256, [1, 3], scope='conv4')
            ...
            net = slim.dropout(net, keep_prob=prob_close(is_train, 0.45), scope='dropout6')
            net = slim.fully_connected(net, 256, scope='fc7')
            net = slim.dropout(net, keep_prob=prob_close(is_train, 0.45), scope='dropout7')
            net = slim.fully_connected(net, 12, activation_fn=None, scope='fc8')
    return net


def net_3(x, is_train):
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        activation_fn=tf.nn.relu,
                        weights_initializer=var_scale,
                        biases_initializer=const_init):
